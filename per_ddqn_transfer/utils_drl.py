...
class Agent(object):
    ...
    def __init__(
        ...
        if restore is None:
            ...
            self.__policy.load_state_dict(torch.load(restore), strict=False)
    ...
    def run_eval(self, state: TensorStack4, training: bool = False) -> int:
        """run suggests an action for the given state, in eval mode."""
        with torch.no_grad():
            return self.__policy(state).max(1).indices.item()

    def learn(self, memory: ReplayMemory, batch_size: int) -> float:
        """learn trains the value network via TD-learning."""
        indices, state_batch, action_batch, reward_batch, next_batch, done_batch, weights_batch = \
        ...
        # loss = loss * torch.Tensor(weights_batch).to(loss)
        ...
        TD_error = expected - values
        memory.update(indices, TD_error + self.__epsilon)
    ...
    def get_feats(self, x):
        return self.__policy.get_feats(x), self.__policy(x)