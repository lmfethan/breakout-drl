...
class MyEnv(object):
    ...
    def evaluate(
        ...
        ep_rewards = []
        frames = []
        for i in range(self.get_eval_lives() * num_episode):
            ...
            while not done:
                ...
                action = agent.run_eval(state)
                obs, reward, done = self.step(action)
                # if i == 2:
                #     print(reward, self.get_action_meanings()[action])
                ep_reward += reward
                obs_queue.append(obs)
                if render:
                    frames.append(self.get_frame())

            print("第{}条命：分数是{}".format(i, ep_reward))
    ...
    def evaluate_visualization(
        ...
        self.__env = self.__env_train
        return np.sum(ep_rewards) / num_episode, frames, obslist
